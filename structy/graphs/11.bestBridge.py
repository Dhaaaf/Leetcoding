...
# My best bridge iterative solutions:
...
def best_bridge(grid):
  ...
  for r in range(len(grid)):
    for c in range(len(grid[0])):
      potential_island = traverse_island(grid, r, c)
  ...
  while queue:
    ...
    for neighbor_row, neighbor_col in [(row + dr, col + dc) for dr, dc in [(1, 0), (-1, 0), (0, 1), (0, -1)]]:
...
def traverse_island(grid, row, col):
    visited = set()
    pos = (row, col)
    stack = [(row, col)]
    while stack:
        ...
        if inbounds(grid, r, c) and grid[r][c] == "L" and pos not in visited:
            visited.add(pos)
            # stack.append((row + 1, col))
            # stack.append((row - 1, col))
            # stack.append((row, col + 1))
            # stack.append((row, col - 1))
            stack.extend([(r + dr, c + dc) for dr, dc in [(1, 0), (-1, 0), (0, 1), (0, -1)]])
